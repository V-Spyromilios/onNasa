...
class OpportunityViewController: UIViewController {
	...
	struct PhotoSection: SectionModelType {
		...
		init(original: OpportunityViewController.PhotoSection, items: [CellItem]) {
			self = original
			self.items = items
		}
	}
	...
	private func createSectionsAndDataSource() {
		...
		let dataSource = RxCollectionViewSectionedReloadDataSource<SectionModel<String, CellItem>>(configureCell: {
			...
			let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "opportunityCollectionCell", for: indexPath) as! OpportunityCollectionCell
		...
		let sections = viewModel.opportunityData
			.map { opportunityData -> [String: [CellItem]] in
				
				guard let opportunityData = opportunityData else { return [:] }
				var itemsDict = [String: [CellItem]]()
				for photo in opportunityData.photos {
